...
class TestIssueStructure:
    ...
    def test_issue_has_id(self, monkeypatch, one_project, filled_issue_list):
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        assert one_project.issues[0].issue_id == "2-1"
        assert one_project.issues[1].issue_id == "2-2"
        assert one_project.issues[2].issue_id == "2-3"

    def test_first_issue_is_type_issue(
        self, monkeypatch, one_project, filled_issue_list
    ):
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        assert isinstance(one_project.issues[0], Issue)

    def test_issues_have_ids(self, monkeypatch, one_project, filled_issue_list):
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        for i in range(0, 3):
            assert one_project.issues[i].issue_id == f"2-{i + 1}"
    ...
    def test_get_issue_has_correct_data(
        ...
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        for number, issue in enumerate(one_project.issues, start=1):
            assert issue.issue_id == f"2-{number}"
            assert issue.id_readable == f"FIRST-{number}"
            assert issue.project_id == "0-1"

    def test_has_timestamps(self, monkeypatch, one_project, filled_issue_list):
        time_stamps = {
            "FIRST-1": {
        ...
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        for issue in one_project.issues:
    ...
    def test_has_summary_and_description(
        ...
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        for number, issue in zip(("first", "second", "third"), one_project.issues):
    ...
    def test_has_comments_count(self, monkeypatch, one_project, filled_issue_list):
        monkeypatch.setattr(request, "urlopen", filled_issue_list)
        for number, issue in zip((0, 1, 42), one_project.issues):
